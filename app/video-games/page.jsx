...
const Library = () => {
  ...
  return (
    <div className='pt-8 flex flex-col items-start gap-16'>
      <h1 className='font-bold text-2xl mt-2 ml-12'>Game List</h1>
      ...
      <div className='w-full max-w-screen-xl grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-10 gap-8 px-4 md:px-10 '>
        ...
        {games.map((game) => (
          <div
            key={game.id}
            className='m-2 border border-gray-300 rounded-md w-24 h-24 flex items-center justify-center overflow-hidden'
          >
            {game.images ? (
              <Link
